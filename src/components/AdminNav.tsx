...
const links = [
  ...
  { href: "/admin/students", label: "บันทึกการสอน" },
...
export default function AdminNav() {
  ...
  return (
    <div
      ref={menuRef}
      className="relative flex items-center gap-3 rounded-xl bg-gradient-to-r from-blue-950 to-blue-900 p-3 shadow-md"
    >
      <button
        onClick={() => setOpen((o) => !o)}
        aria-label="เมนู"
      ...
      {open && (
        <div className="absolute left-0 top-full z-10 mt-2 w-52 space-y-1 rounded-xl border border-gray-200 bg-white p-2 text-sm shadow-lg">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              ...
              className={`block rounded-lg px-3 py-2 hover:bg-orange-50 ${
                pathname === link.href ? "bg-orange-50 font-medium text-orange-700" : ""
              }`}
            >
              {link.label}
            </Link>
